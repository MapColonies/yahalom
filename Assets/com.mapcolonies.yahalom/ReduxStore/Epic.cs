...
{
    public delegate Observable<IAction> Epic<TState, in TDependencies>(
        Subject<IAction> action,
        ReadOnlyReactiveProperty<TState> state,
        TDependencies dependencies
    );

    public delegate Observable<IAction> Epic<TState>(
        Subject<IAction> action,
        ReadOnlyReactiveProperty<TState> state
    );
    ...
    {
        public static Epic<TState> Create<TState>(Func<Subject<IAction>, ReadOnlyReactiveProperty<TState>, Observable<IAction>> epic)
        {
            return (action, state) => epic(action, state);
        }

        public static Epic<TState> Create<TState>(string actionType, Func<Observable<IAction>, ReadOnlyReactiveProperty<TState>, Observable<IAction>> epic)
        {
            return Create<TState>((action, state) =>
                epic(action.OfAction(actionType), state));
        }

        public static Epic<TState> Create<TState>(ActionCreator action, Func<Observable<Action>, ReadOnlyReactiveProperty<TState>, Observable<Action>> epic)
        {
            return Create(action.type, epic);
        }

        public static Epic<TState, TDependencies> Create<TState, TDependencies>(Func<Observable<IAction>, ReadOnlyReactiveProperty<TState>, TDependencies, Observable<IAction>> epic)
        {
            return (action, state, dependencies) => epic(action, state, dependencies);
        }

        public static Epic<TState, TDependencies> Create<TState, TDependencies>(string actionType, Func<Observable<IAction>, ReadOnlyReactiveProperty<TState>, TDependencies, Observable<IAction>> epic)
        {
        ...
        public static void AddTo<TState, TDependencies>(this Epic<TState, TDependencies> epic, ref EpicBuilder<TState, TDependencies> builder)
        {
            builder.Add(epic);
        }
        ...
        public static Epic<TState, TDependencies> Combine<TState, TDependencies>(params Epic<TState, TDependencies>[] epics)
        {
